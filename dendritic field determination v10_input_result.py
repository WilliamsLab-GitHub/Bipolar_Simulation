...
def enriching_cell(DScell, resolution):
    ...
    for sec in DScell:
        ...
        if dis <= resolution:
            newcontour.append([xs, ys, xe, ye, sec[4], sec[5]])
        else:
...
print('reading bipolar arrays...')

################################################################################

#filepath = 'C:/Users/uqxzho15/Desktop/py code/bipolar sim 3 sustained off alpha/raw data/unrotated test'
#filepath = 'C:/Users/uqxzho15/Desktop/py code/bipolar sim 3 sustained off alpha/raw data/data all/alpha'
#filepath = 'C:/Users/uqxzho15/Desktop/py code/bipolar sim 3 sustained off alpha/raw data/data all/sustained'
filepath = 'C:/Users/uqxzho15/Desktop/py code/bipolar sim 3 sustained off alpha/raw data/sustained add one'
...
for inputfile in filelist:
    ...
    for cell in DScell:
        if cell in anglemaxlist:
            morph.append([cell[2], cell[3], ' ', cell[2], cell[3]])
        else:
            morph.append([cell[2], cell[3]])

    book = xlwt.Workbook(encoding="utf-8")
    sheet1 = book.add_sheet("Sheet 1")
    for p in range(len(morph)):
        for q in range(len(morph[p])):
            sheet1.write(p,q,morph[p][q])
    morphname = cellname + ' morph.xls'
    book.save(morphname)
    """
    ################################################################################

    #morph_path = 'C:/Users/uqxzho15/Desktop/py code/bipolar sim 3 sustained off alpha/batch 14 dend v10 manual/fixed morph/alpha/'
    #morph_path = 'C:/Users/uqxzho15/Desktop/py code/bipolar sim 3 sustained off alpha/batch 14 dend v10 manual/fixed morph/sustained/'
    morph_path = 'C:/Users/uqxzho15/Desktop/py code/bipolar sim 3 sustained off alpha/batch 14 dend v10 manual/fixed morph/sustained add one/'
    morph_fixed_name = cellname + ' morph fixed.xls'
    morph_input = morph_path + morph_fixed_name
    morph_wb = xlrd.open_workbook(morph_input)
    
    for n in range(len(morph_wb.sheet_names())):
        if morph_wb.sheet_names()[n] == 'Sheet 1':
            cell_morph = morph_wb.sheets()[n]
            cell_line = []
            for row in range(cell_morph.nrows):
                col_value = []
                for col in range(cell_morph.ncols):
                    col_value.append(cell_morph.cell(row,col).value)
                cell_line.append(col_value)    
    ...
    for cell in cell_line:
        if len(str(cell[3])) != 0:
            x = float(cell[3])
            y = float(cell[4])
            dend_contour_random.append([x,y,math.atan2(y,x)/math.pi*180])
    print('reading fixed dend contour...')
            
    dend_contour_sequence_angle = sorted(dend_contour_random, key = lambda cell: cell[2])
    print('processing dend contour...')
    
    dend_contour_sequence = []
    for dend in dend_contour_sequence_angle:
        dend_contour_sequence.append([dend[0], dend[1]])
    
    dend_contour_sequence.append(dend_contour_sequence[0])
    dend_contour_seq_cell = []
    for i in range(len(dend_contour_sequence)-1):
        xs = dend_contour_sequence[i][0]
        ys = dend_contour_sequence[i][1]
        xe = dend_contour_sequence[i+1][0]
        ye = dend_contour_sequence[i+1][1]
        avg_dia = 0.3
        p_type = ' '
        cell = [xs, ys, xe, ye, avg_dia, p_type]
        dend_contour_seq_cell.append(cell)
    print('dend contour originally has', len(dend_contour_seq_cell), 'points')
    
    dend_contour_seq_cell_enriched = enriching_cell(dend_contour_seq_cell, 6)
    print('enriching dend contour...')
    print('now dend contour has', len(dend_contour_seq_cell_enriched), 'points')
    
    dendfield = []        
    for cell in dend_contour_seq_cell_enriched:
        dendfield.append([0,0,cell[0],cell[1],1])
    print('dendfield completed...')
    ...
    ################################################################################
        ...
        if n_plot == 0:
            ...
            for cell in dend_contour_seq_cell:
                line1 = [(cell[0], cell[1]), (cell[2], cell[3])]
                (line1_xs, line1_ys) = zip(*line1)
                line = Line2D(line1_xs, line1_ys, linewidth=cell[4], color='red')
                line.set_zorder(0)
                ax.add_line(line)
            """
            for cell in dendfield:
                line1 = [(cell[0], cell[1]), (cell[2], cell[3])]
                (line1_xs, line1_ys) = zip(*line1)
                line = Line2D(line1_xs, line1_ys, linewidth=0.1, color='yellow')
                line.set_zorder(0)
                ax.add_line(line)
            """
            ax.axis('off')
        ...
        ################################################################################
            ...
            for cell in dend_contour_seq_cell:
                line1 = [(cell[0], cell[1]), (cell[2], cell[3])]
                (line1_xs, line1_ys) = zip(*line1)
                line = Line2D(line1_xs, line1_ys, linewidth=cell[4], color='blue')
                line.set_zorder(0)
                ax.add_line(line)
